...
def greedy_algorithm(snapshots,NumerOfTimesSteps,snapshotCorrelationOperator, NumberOfModes):
    ...
    TolGreedy=5e-5 #Tolerance on the time steps
    DegreesOfFreedom=np.shape(snapshotCorrelationOperator)[0]
    print("DoFs",DegreesOfFreedom)
    NbParam=len(snapshots)
    print("NumberOfParameters",NbParam)
    ...
    while(NumberOfModes>cptmodes-1):
        if cptmodes+1==NumberOfModes:
            break
        ...
        #Gram-Schmidt
        orthogonality=orthogonality_check(np.reshape(basis_vectors,((cptmodes+1,m))),snapshotCorrelationOperator)
        orthocpt=0
        while(10>=orthocpt and orthogonality==False):
            basis_vectors=orthoGS(basis_vectors,cptmodes+1,m,snapshotCorrelationOperator)
        ...
        TimeIndices.append(int(Index/(NbParam-cptParam)+0.00001))
        #print("Time", int(Index/(NbParam-cptParam)+0.00001))
        while(3>=cpt and tol >= TolGreedy and NumberOfModes> cptmodes-1):
        #while(4>=cpt and NumberOfModes > cptmodes): #greedy on time steps
            ...
            NormTestMax = np.sqrt(np.diag(np.dot(residuals,matVecProduct)))
            TimeIndex = np.argmax(NormTestMax)
            #print("TimeIndex argmax", TimeIndex)
            mask = (TimeIndex not in TimeIndices)
            if mask==False: #check if not already in the time steps 
    ...
    """
        ...
        RB=np.array(RB).reshape(NumberOfModes,DegreesOfFreedom)
...
def GreedyNew(snapshots,NumberOfTimeSteps,snapshotCorrelationOperator,GlobalIndex,h1ScalarProducMatrix=None,NumberOfModes=0,Tol=1e-30): ##create a RB from a priori given parameters
    ...
    #print("shape",np.shape(snapshotIterator1))
    TimeIndex=[indTime]
